...
if st.button('Run') or state['data_received']:
    ...
    #Création d'une jaune pour voir où se positionne le client par rapport au threshold
    jauge_html = f"""
    ...
    for bar, feature_value in zip(bars_left, shap_df_decrease['Feature Value']):
        axes[0].text(bar.get_width() + 0.01, #Dans la barre
    ...
    axes[1].invert_yaxis() #Inverse l'axe pour que la feature la plus importante soit en haut
    ...
    for bar, feature_value in zip(bars_right, shap_df_increase['Feature Value']):
        axes[1].text(bar.get_width() - 0.01, #Dans la barre
                     bar.get_y() + bar.get_height() / 2, #Centré dans la barre
                     f'{feature_value}',
